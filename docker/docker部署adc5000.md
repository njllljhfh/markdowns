...
# -------------------------- 拷贝项目到宿主机 --------------------------
# 创建项目目录
mkdir -p /home/$(whoami)/projects/adc5000/v5000
mkdir -p /home/$(whoami)/projects/adc5000/v5000/html

# 将前端静态文件拷贝到宿主机目录
/home/$(whoami)/projects/adc5000/v5000/html

# 拉取项目
cd /home/$(whoami)/projects/adc5000/v5000/ &&
git clone git@192.168.10.30:dev/MS_ADC.git &&
cd MS_ADC &&
git checkout dev_1.1.9_quliang &&
chmod 777 auto_start_uwsgi_celery.sh start_celery.sh start_uwsgi.sh mysql_init.sh &&
cp uwsgi.ini_docker uwsgi.ini &&
cd /home/$(whoami)/projects/adc5000/v5000/MS_ADC/config/settings &&
\cp -rf production_docker_network.py production.py &&
cd -

# 修改宿主机项目目录中 production.py 文件
vim /home/$(whoami)/projects/adc5000/v5000/MS_ADC/config/settings/production.py
# 修改ip
HOST_IP="宿主机ip"
...
cd /home/$(whoami)/projects/adc5000/v5000/MS_ADC
docker compose up -d
...
排序规则 utf8_bin
# 运行初始化表的sql
/home/$(whoami)/projects/adc5000/v50006000/MS_ADC/builds/data_and_structure.sql
# 运行初始化缺陷类的sql
/home/$(whoami)/projects/adc5000/v50006000/MS_ADC/builds/generate_lab_class/lab_class_generated.sql
...
# dockerfile 创建镜像
# adc 服务
docker build -t adc_server:django_py3.8_uwsgi -f adc_server.dockerfile .
# adc server 启动容器  进入容器
docker run -itd --name django_uwsgi -p 8000:7156 adc_server:django_py3.8_uwsgi && docker exec -it django_uwsgi /bin/bash
#----
docker stop django_uwsgi && docker rm django_uwsgi



# mysql 创建镜像
docker build -t mysql:mysqldb -f mysql.dockerfile .
# mysql 启动容器  进入容器
docker run -d -p 3306:3306 --name mysqldb -e MYSQL_ROOT_PASSWORD=mysql mysql:mysqldb && docker exec -it mysqldb /bin/bash
# ---
docker exec -it mysqldb /bin/bash
...
create user 'adc5000'@'%' identified by 'mysql';
grant all privileges on *.* to 'adc5000'@'%';
#---
service mysql stop
service mysql start
...
# redis_mq_minio
docker build -t componet_server:redis_mq_minio -f componet_server.dockerfile .
# redis_mq_minio 启动容器  进入容器
docker run -itd --name redis_mq_minio -p 6379:6379 -p 9000:9000 -p 60006:60006 -p 5672:5672 -p 15672:15672 componet_server:redis_mq_minio && docker exec -it redis_mq_minio /bin/bash
#----
docker stop redis_mq_minio && docker rm redis_mq_minio

docker run -itd --name redis_mq_minio -p 9379:6379 componet_server:redis_mq_minio

redis_pids=$(ps aux | grep minio | grep -v grep | awk '{print $2}' | tr -s '\n' ' ')
minio_pids=$(lsof -i:9000 | grep -v grep | awk '{print $2}' | tr -s '\n' ' ')
...
# 停止容器，删除容器
docker stop $(docker ps -a -q) && docker rm $(docker ps -a -q) && docker ps -a && docker images
